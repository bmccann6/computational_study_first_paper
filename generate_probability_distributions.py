...
def update_bin_dict(bins_data, prob_dist, entropy):
    ...
    for (lower_bound, upper_bound), bin_data in bins_data.items():
        ...
        if len(bin_data) == NUM_SAMPLES_PER_BIN:
            break
...
def main(stdscr):
    ...
    max_num_while_loop_iterations_before_increasing_power = NUM_SAMPLES_PER_BIN * NUM_BINS
    ...
    for i in range(NUM_BINS - 1, -1, -1):
        ...
        while len(bins_data[bin_key]) < NUM_SAMPLES_PER_BIN:
            ...
            bin_counts = {k: (len(v) if len(v) != NUM_SAMPLES_PER_BIN else "All samples obtained.") for k, v in bins_data.items()}
            ...
            all_higher_bins_done = all( len(bins_data[(j/NUM_BINS, (j+1)/NUM_BINS)]) == NUM_SAMPLES_PER_BIN for j in range(i, NUM_BINS) )
...
if __name__=="__main__":
    ...
    with open(f"generated_prob_dist_data/prob_distributions_generated_NUM_SAMPLES_PER_BIN_{NUM_SAMPLES_PER_BIN}_and_NUM_BINS_{NUM_BINS}.pkl", "wb") as file:
