...
# NOTE: These are the A_i values from backloading. Not the expected value of items at each node i.
...
def calculate_expected_value_item_each_node_and_breaking_indices_specific_resource_set(resource_set_dict):
    ...
    # # Return both the amount_value_detected and the breaking_indices so the caller can log them
    # return amount_value_detected, breaking_indices
...
#                 dict_bins_and_values[key].append(expected_value_detected_this_run)
...
# def update_bin_dict(dict_bins_and_values, entropy, expected_value_detected_this_run, expected_total_value_this_run):
#     for outer_key in dict_bins_and_values:
#         if outer_key[0] <= entropy < outer_key[1]:
#             if isinstance(dict_bins_and_values[outer_key], dict) and (len(dict_bins_and_values[outer_key]["expected_values_detected"]) < required_num_samples_per_bin):
#                 dict_bins_and_values[outer_key]["expected_values_detected"].append(expected_value_detected_this_run)
#                 dict_bins_and_values[outer_key]["expected_total_values"].append(expected_total_value_this_run)
#                 if len(dict_bins_and_values[outer_key]["expected_values_detected"]) == required_num_samples_per_bin:    # Now we will replace the dictionary for this bin with just the value needed for this bin.
#                     dict_bins_and_values[outer_key] = dict_bins_and_values[outer_key]["expected_values_detected"] / dict_bins_and_values[outer_key]["expected_total_values"]
#             break
#     return dict_bins_and_values

#     \ici Maybe to simplyify everything, we don't need to replace the outer_key value with the float. But we can just have another key in the end that we create called "final_value".
#     Then we just check if "final_value" is a key in dict_bins_and_values[outer_key].
